# M3B: Pilot Implementation Report — DIDs for Roles, Authentication & Access Management
...
| Consumer | Environment variable | Notes |
|----------|---------------------|-------|
| enrol (`secureDIDCheck`) | `NEXT_PUBLIC_DID_INDEXER_URL` | SSRF-safe hostname whitelist |
| docs-secret | `INDEXER_URL_MAINNET` / `INDEXER_URL_PREPROD` | Network-aware resolution |
| enrol (planned) | `DID_INDEXER_ENDPOINT` | Future `did-lookup.ts` integration |

See [API.md](./API.md) and [P1_INDEXER_PLAN.md](./P1_INDEXER_PLAN.md) for indexer implementation detail.
...
**Current state (V1):** the enrol auth endpoint resolves authorization from `PROPAGATE_AUTH_WALLETS` (address:role allowlist). The planned V2 path in `enrol/src/lib/auth/did-lookup.ts` will derive the DID from the wallet address and resolve `hubRole` from the indexer via `@prisma-events/dids-sdk`.
...
**Secret host:** `PRIVATE_DOCS_API_ONLY=true` — serves auth APIs and full private content
...
- `access` → minimum role required (defaults to `general-participant`)

Examples in the repo:

| Page | `access` |
|------|----------|
| `practice-currencies.mdx` | `participant` |
| `product-owner.mdx` (contract) | `general-participant` |
...
|-------|-----------------|-----------------|
| Page content | `<PrivatePageShell />` stub only | Full markdown/MDX |
...
Roles like `participant` and `general-participant` reflect **event-scoped enrolment**, not platform-wide RBAC. A wallet's role may differ across events as enrolment records and DID document updates reflect cohort participation.
...
| **Role granularity** | Hub-level (`OWNER`, `MEMBER`) | Page-level (`participant`, `general-participant`, …) |
...
- [x] CIP-30 wallet signature verification (Propagate core + both web apps)
- [x] DID derivation from stake address (`did:cardano:…`)
- [x] Indexer queries in docs-secret `did-prepare` and enrol `secureDIDCheck`
- [x] Propagate CLI login flow with enrol wallet authorization (V1 allowlist)
- [x] Docs split public/private export with wallet-gated content fetch
- [x] Session issuance with embedded `{ did, role }` in docs-secret

### In progress / planned

- [ ] **enrol `did-lookup.ts` V2** — replace stub with SDK derive + indexer lookup for Propagate hub roles
- [ ] **docs-secret role enforcement** — `serve-private` reads `requiredRole` from frontmatter but does not yet compare against `session.role` (403 path exists on client, server logs only)
- [ ] **Role hierarchy** — no comparison function yet; string equality assumed
- [ ] **On-chain manifest** — Propagate stack manifest anchoring (future version)
- [ ] **Unified role vocabulary** — map event-scoped enrolment roles to hub roles where operators are also participants
...
| Resource | Path / URL |
|----------|------------|
| Propagate README | `propagate/README.md` |
| Propagate auth core | `propagate/packages/core/src/auth.ts` |
| Propagate CLI login | `propagate/packages/cli/src/auth/remote-login.ts` |
| Propagate web verify | `propagate/packages/web/src/app/api/auth/verify/route.ts` |
| Enrol auth API | `enrol/src/app/api/auth/[address]/route.ts` |
| Enrol DID lookup (planned) | `enrol/src/lib/auth/did-lookup.ts` |
| Enrol DID check | `enrol/src/lib/securedid/secureDIDCheck.ts` |
| Docs public/private guide | `docs-secret/content/en/processes/process-infrastructuring/publishing/docs/public-private.md` |
| Docs auth prepare | `docs-secret/app/api/auth/did-prepare/route.ts` |
| DID indexer API | [API.md](./API.md) |
| Indexer implementation plan | [P1_INDEXER_PLAN.md](./P1_INDEXER_PLAN.md) |
| Technical design | [TECHNICAL_DESIGN_1.6.2.md](./TECHNICAL_DESIGN_1.6.2.md) |
